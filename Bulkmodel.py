...
LGBM_model = joblib.load("finalized_LGBM.sav")
...
def visuals2(df):
    ...
    sns.barplot(x =df["marital"], y =df["age"],hue = df["y"],ax= axs[0])
    sns.barplot(x = df["education"], y =df["age"],hue = df["y"], ax= axs[1])
    #sns.factorplot(x = df["y"], y =df["age"],hue = df["education"])
    #sns.factorplot(x = df["y"], y =df["age"],hue = df["job"],ax= axs[1][1])
...
def visuals3(df):
    ...
    sns.factorplot(x = "y", y ="age",hue ="job",data=df)
    plt.show()
...
def visuals4(df):
    ...
    sns.factorplot(x ="y", y ="age",hue = "education",data = df)
    plt.show()

def Three_dimension_plot(x_pca):
    # for interactive visualizations
    #pca = PCA(3)
    #x_pca = pca.fit_transform(df[['age','campaign','emp.var.rate']])
    #x_pca = pd.DataFrame(x_pca)
    #x_pca.columns = ['pc1', 'pc2', 'pc3']
    x_pca["y"] = x_pca["y"].replace(['yes','no'],[1,0])
    trace1 = go.Scatter3d(x= x_pca['emp.var.rate'],y= x_pca['age'],z= x_pca['campaign'],
                          mode='markers',marker=dict(color = x_pca['y'],
                                                     size= 10,line=dict(color= x_pca['y'],
                                                                        width= 12),opacity=0.8))
    dt = [trace1]
    layout = go.Layout(title = 'age vs campgain vs accept or not',
                       margin=dict(l=0,r=0,b=0,t=0),scene = dict(xaxis = dict(title  = 'emp.var.rate'),yaxis = dict(title  = 'age'),zaxis = dict(title  = 'campaign')))
    fig = go.Figure(data = dt, layout = layout)
    py.iplot(fig)
    #trace1 = go.Scatter3d(x= x_pca['pc1'],y= x_pca['pc2'],z= x_pca['pc3'],mode='markers',
    #                      marker=dict(color = x_pca['Label'], size= 10,line=dict(color= x_pca['Label'],width= 12),
     #                                 opacity=0.8))
    #dt = [trace1]
    
     
    
    #layout = go.Layout(title = 'Character vs Gender vs Alive or not',margin=dict(l=0,r=0,b=0,t=0),
...
def stirplot(df):
    for i,val in enumerate(df["age"]):
        if val <= 30 and val >=16:
            df.loc[i,"age_bin"] = "young"
        elif val > 30 and val <= 60:
            df.loc[i,"age_bin"] = "middle"
        elif val > 60:
            df.loc[i,"age_bin"] = "old"
    df["y"] = df["y"].replace(['yes','no'],[1,0])
    sns.stripplot(x="age_bin", y="campaign",data=df,hue='y',jitter=True,split=True)
    plt.title("age_bin vs campaign")
...
def main():
    ...
    if choice == "Insightful EDA":
        ...
        if data is not None:
            ...
            df = pd.read_csv(data,sep=';')
            if st.button("visual1"):
                st.pyplot(visuals1(df))
                
            if st.button("visual2"):
                st.pyplot(visuals2(df))
            if st.button("visual3"):
                st.pyplot(visuals3(df))
            if st.button("visual4"):
                st.pyplot(visuals4(df))
            if st.button("visual5"):
                st.pyplot(stirplot(df))
